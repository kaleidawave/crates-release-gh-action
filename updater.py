...
local_pkgs = set()
awaiting = dict()
publishable_packages = list()

for package in packages:
    if isinstance(package["publish"], list) and len(package["publish"]) == 0:
        continue
    ...
    for dependency_name in dependency_names.copy():
        if dependency_name in local_pkgs:
            if dependency_name in flat_packages:
                ...
                awaiting.setdefault(dependency_name, []).append(name)

    for awaitee in awaiting.pop(name, []):
        flat_packages[awaitee].extend(dependency_names)

    flat_packages[name] = dependency_names
    local_pkgs.add(name)
    publishable_packages.append(package)
...
packages = sorted(publishable_packages, key=cmp_to_key(package_order))
...
def bump_version(existing_version, behavior):
    match behavior:
        ...
        case _:
            raise ValueError(
                f"Expected 'patch', 'minor', 'major' or 'none'. Found '{behavior}'"
            )
...
if argument.startswith("{"):
    ...
    stream = sys.stdout if dry_run else open(environ["GITHUB_OUTPUT"], "w")
    with stream as f:
        print("new-version=none", file=f)
        print(f"new-versions=[{changes}]", file=f)
        print(
            f"new-versions-description={format_change_list(updated_crates.items())}",
            file=f,
        )
    ...
    if len(packages) == 1:
        ...
        with stream as f:
            ...
            print(
                f'new-versions-json-object={{"{crate_name}":"{new_version}"}}', file=f
            )
